...
#include "sgmm3dnn/estimate-am-sgmm2.h"
...
int main(int argc, char *argv[]) {
  try {
    ...
    string gselect_rspecifier, gammar_rspecifier, spk2utt_rspecifier, spkvecs_rspecifier;
    ...
    RandomAccessInt32VectorVectorReader gselect_reader(gselect_rspecifier);
     //////// read the posteriors as matrix as we do not have RandomAccessBaseFloatVectorVectorReader
	  ...
	  const Matrix<BaseFloat>  &gammar = 
	     gammar_reader.Value(utt);
          ...
          AccumulateForUtterance(feats, post, trans_model, am_sgmm,
                                 gselect, gammar, &spk_vars, &spk_stats);
          num_done++;
